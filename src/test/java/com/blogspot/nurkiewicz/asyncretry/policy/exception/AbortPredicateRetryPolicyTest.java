...
public class AbortPredicateRetryPolicyTest extends AbstractExceptionClassRetryPolicyTest {
	...
	public void shouldAbortIfPredicateTrue() throws Exception {
		//given
		final RetryPolicy retryPolicy = new AbortPredicateRetryPolicy(always, new Predicate<Throwable>() {
			@Override
			public boolean apply(java.lang.Throwable input) {
				return true;
			}
		});
	...
	public void shouldProceedIfPredicateFalseAndChildAccepts() throws Exception {
		//given
		final RetryPolicy retryPolicy = new AbortPredicateRetryPolicy(retryPolicyMock, new Predicate<Throwable>() {
			@Override
			public boolean apply(java.lang.Throwable input) {
				return false;
			}
	...
	public void shouldExamineExceptionAndDecide() throws Exception {
		...
		final RetryPolicy retryPolicy = new AbortPredicateRetryPolicy(retryPolicyMock, new Predicate<Throwable>() {
			@Override
			public boolean apply(java.lang.Throwable input) {
				return input.getMessage().contains("abort");
			}
		});
		given(retryPolicyMock.shouldContinue(notNullRetryContext())).willReturn(true);
